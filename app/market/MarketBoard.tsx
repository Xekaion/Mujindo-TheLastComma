    ...
    switch (confirmation.kind) {
      case "buy":
        await executeCommand(
          ...
          `${confirmation.listing.item.displayName} 구매를 완료했습니다.`,
        );
        return;
      case "cancel-listing":
        await executeCommand(
      ...
      case "sell":
        await executeCommand(
          ...
          `${confirmation.item.displayName} 판매 등록을 완료했습니다.`,
        );
        return;
      case "order":
        await executeCommand(
                ...
                <section className="market-listings" aria-label="판매 장비 목록">
                  ...
                  {listings.length === 0 ? <EmptyState title="조건에 맞는 매물이 없습니다" body="검색 조건을 바꾸거나 다음 실시간 갱신을 기다려 주세요." /> : listings.map((listing) => (
                    <article key={listing.listingId} className={`market-listing is-${listing.item.rarity}`} style={{ "--rarity-color": RARITY_COLORS[listing.item.rarity] } as CSSProperties}>
                      <div className="market-listing-item"><ItemIcon item={listing.item} compact /><div><small>LV.{listing.item.level} · +{listing.item.enhancement} · {SLOT_LABELS[listing.item.slot]}</small><strong>{listing.item.displayName}</strong><span>{RARITY_LABELS[listing.item.rarity]} · 전투력 {formatEconomyAmount(listing.item.powerScore)} · 품질 {listing.item.qualityScore}</span></div></div>
...
function getConfirmationCopy(confirmation: Confirmation): { title: string; body: string; confirmLabel: string; rows: Array<[string, string]> } {
  switch (confirmation.kind) {
    case "buy": return { title: `${confirmation.listing.item.displayName}을 구매할까요?`, body: "구매가 완료되면 장비는 서버 금고로 이동하고 판매 대금은 판매자에게 정산됩니다.", confirmLabel: "기억의 재로 구매", rows: [["구매 가격", `✦ ${formatEconomyAmount(confirmation.listing.priceAsh)}`], ["판매자", confirmation.listing.sellerName], ["장비", `LV.${confirmation.listing.item.level} ${RARITY_LABELS[confirmation.listing.item.rarity]}`]] };
    case "cancel-listing": return { title: "판매 등록을 취소할까요?", body: "아직 체결되지 않은 매물만 취소할 수 있으며 장비는 서버 금고로 반환됩니다.", confirmLabel: "등록 취소", rows: [["장비", confirmation.listing.item.displayName], ["등록가", `✦ ${formatEconomyAmount(confirmation.listing.priceAsh)}`]] };
    case "sell": return { title: `${confirmation.item.displayName}을 판매할까요?`, body: "등록되는 순간 장비는 거래 보관 상태가 되며 원정이나 다른 거래에 사용할 수 없습니다.", confirmLabel: "판매 등록", rows: [["판매 가격", `✦ ${formatEconomyAmount(confirmation.priceAsh)}`], ["장비 전투력", formatEconomyAmount(confirmation.item.powerScore)], ["등급", RARITY_LABELS[confirmation.item.rarity]]] };
